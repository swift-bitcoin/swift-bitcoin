...
func signSchnorr(msg: Data, secretKey: Data, merkleRoot: Data?, skipTweak: Bool = false, aux: Data?) -> Data {
    ...
    let sig: [UInt8] = .init(unsafeUninitializedCapacity: 64) { buf, len in
        let successOrError = signSchnorr(eccSigningContext, computeTapTweakHashExtern(_:_:_:), buf.baseAddress, &len, msgPtr, merkleRootPtr, skipTweak ? 1 : 0, auxPtr, secretKeyPtr)
