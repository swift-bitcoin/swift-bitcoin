...
public func getPublicKey(secretKey secretKeyData: Data, compress: Bool = true) -> Data {
    ...
    var pubKeyBytes = [UInt8](repeating: 0, count: compress ? compressedPublicKeySize : uncompressedPublicKeySize)
    ...
    assert(compress && pubKeyBytesCount == compressedPublicKeySize || (!compress && pubKeyBytesCount == uncompressedPublicKeySize))
