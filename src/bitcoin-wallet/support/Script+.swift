...
extension Script {
    ...
    var isPayToPubkey: Bool {
        if (dataSize == PublicKey.compressedLength + 2 || dataSize == PublicKey.uncompressedLength + 2),
           ops.count == 2,
    ...
    package var isPayToPubkeyHash: Bool {
        if dataSize == RIPEMD160.Digest.byteCount + 5,
           ops.count == 5,
           ops[0] == .dup,
           ops[1] == .hash160,
    ...
    var isPayToTaproot: Bool {
        if dataSize == PublicKey.xOnlyLength + 2,
           ops.count == 2,
           ops[0] == .constant(1),
    ...
    var isPayToWitnessKeyHash: Bool {
        if dataSize == RIPEMD160.Digest.byteCount + 2,
           ops.count == 2,
           ops[0] == .zero,
           case .pushBytes(_) = ops[1] { true } else { false }
    }
    ...
    var isPayToWitnessScriptHash: Bool {
        if dataSize == SHA256.Digest.byteCount + 2,
           ops.count == 2,
           ops[0] == .zero,
           case .pushBytes(_) = ops[1] { true } else { false }
    }
