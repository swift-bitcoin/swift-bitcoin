...
public indirect enum TapscriptTree: Equatable, Sendable {
    ...
    public init(_ scripts: [Data], leafVersion v: Int) {
        ...
        case 1:
            TapscriptTree.leaf(v, scripts[0])
        case 2:
            TapscriptTree.branch(
                .leaf(v, scripts[0]),
                .leaf(v, scripts[1]))
        case 3:
            TapscriptTree.branch(
    ...
    public func calcMerkleRoot() -> ([(TapscriptTree, Data)], Data) {
        switch self {
        case .leaf(_, _):
    ...
    public var leafs: [TapscriptTree] { leafs() }

    private func leafs(_ partial: [TapscriptTree] = []) -> [TapscriptTree] {
        switch self {
        case .leaf(_, _):
            return partial + [self]
