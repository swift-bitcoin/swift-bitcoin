...
public struct SignatureHasher: Sendable {

    public init(tx: Transaction, input: Int, sigVersion: SigVersion = .base, prevout: TransactionOutput, scriptCode: Data? = .none, tapscriptExtension: TapscriptExtension? = .none, sighashType: SighashType = .all) {
        precondition(input < tx.ins.count)
    ...
    public init(tx: Transaction, input: Int = 0, sigVersion: SigVersion = .base, prevouts: [TransactionOutput], scriptCode: Data? = .none, tapscriptExtension: TapscriptExtension? = .none, sighashType: SighashType? = Optional.none) {
        precondition(input < tx.ins.count)
    ...
    public mutating func set(input: Int, sigVersion: SigVersion? = .none, prevout: TransactionOutput) {
    ...
    public mutating func set(input: Int, sigVersion: SigVersion? = .none, prevouts: [TransactionOutput]? = .none, scriptCode: Data? = .none, tapscriptExtension: TapscriptExtension? = .none, sighashType: SighashType?) {
    ...
    public mutating func set(input: Int, sigVersion: SigVersion? = .none, prevouts newPrevouts: [TransactionOutput]? = .none, scriptCode: Data? = .none, tapscriptExtension: TapscriptExtension? = .none) {
        self.scriptCode = scriptCode
        self.tapscriptExtension = tapscriptExtension
        self.inIndex = input
        if let newPrevouts {
        ...
        if sighashType.hasAnyCanPay {
            ...
            tx.ins.enumerated().forEach { i, input in
                newIns.append(.init(
                    ...
                    sequence: i == inIndex || (!sighashType.isNone && !sighashType.isSingle) ? input.sequence : .initial,
    ...
    private var sigMessageSegwit: Data {
        ...
        if prevout.script.isSegwit, prevout.script.witnessProgram.count == Hash160.Digest.byteCount {
            resolvedScriptCode = Script.segwitPKHScriptCode(prevout.script.witnessProgram).binaryData
    ...
    /// BIP341
    func sigHashSchnorr(sighashCache: inout Cache) -> Data {
    ...
    func sigMessageSchnorr(sighashCache: inout Cache) -> Data {
