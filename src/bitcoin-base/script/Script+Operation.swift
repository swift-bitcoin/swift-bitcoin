...
extension Script.Operation {
    ...
    public static func encodeMinimally(_ value: Int) -> Script.Operation {
        switch value {
        case -1:
            return .oneNegate
        case 0:
...
extension Script.Operation: BinaryCodable, ExpressibleByParsing {

    public init(parsing input: inout ParserSpan) throws {
        // decoder.setCheckpoint()
        let range = input.parserRange

        let opCode = try UInt8(parsing: &input)
        switch opCode {
        ...
        case 0x01 ... 0x4b:
            do {
                let payload = try Data([UInt8](parsing: &input, byteCount: Int(opCode)))
                self = .pushBytes(payload)
            } catch {
                // decoder.revert()
        ...
        case 0x4c:
            do {
                ...
                let payload = try Data([UInt8](parsing: &input, byteCount: Int(count)))
                self = .pushData1(payload)
            } catch {
                // decoder.revert()
                try input.seek(toRange: range)
                throw error
