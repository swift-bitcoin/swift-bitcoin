...
public actor BlockchainService: Sendable {
    ...
    private func processBlockInternal(_ block: Block, immediate: Bool = true, locator: BlockStorageLocator?) async throws(Error) {
        ...
        if let ref = await blockIndex.get(block.id) {
            ...
            headerRef = try await processHeaderInternal(block.header, previousHeader: prev)
        }
    ...
    public func processHeader(_ header: Block) async throws(Error) {
        ...
        _ = try await processHeaderInternal(header, previousHeader: prev)
    }
    ...
    private func processHeaderInternal(_ header: Block, previousHeader: BlockRef) async throws(Error) -> BlockRef {
        ...
        for (h, loc) in heldBlocks.values.filter({ $0.0.previous == header.id }) {
            let ref = try await processHeaderInternal(h.header, previousHeader: newHeader)
            if !h.txs.isEmpty {
