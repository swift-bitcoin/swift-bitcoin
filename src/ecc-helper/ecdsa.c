...
char SigHasLowR(const secp256k1_context* ctx, const secp256k1_ecdsa_signature* sig)
{
...
const int signECDSA(const secp256k1_context* ctx, u_char* sigOut, size_t* sigLenOut, const u_char* msg32, const u_char* secretKey32, const u_char grind) {
    ...
    uint32_t counter = 0;
    int ret = secp256k1_ecdsa_sign(ctx, &sig, msg32, secretKey32, secp256k1_nonce_function_rfc6979, (!grind && test_case) ? extra_entropy : NULL);
    // Grind for low R
    while (ret && !SigHasLowR(ctx, &sig) && grind) {
        WriteLE32(extra_entropy, ++counter);
        ret = secp256k1_ecdsa_sign(ctx,  &sig, msg32, secretKey32, secp256k1_nonce_function_rfc6979, extra_entropy);
    }
    assert(ret);
    size_t sigLen = SIGNATURE_SIZE;
    u_char *sigBytes = malloc(sigLen);
    ret = secp256k1_ecdsa_signature_serialize_der(ctx, sigBytes, &sigLen, &sig);
    assert(ret);
    // Additional verification step to prevent using a potentially corrupted signature
    secp256k1_pubkey pk;
    ret = secp256k1_ec_pubkey_create(ctx, &pk, secretKey32);
    assert(ret);
...
const int verifyECDSASecretKey(const secp256k1_context* ctx, const u_char *sigBytes, const size_t sigLen, const u_char* msg32, const u_char* secretKey32) {
    secp256k1_context *secp256k1_context_verify = secp256k1_context_create(SECP256K1_CONTEXT_VERIFY);
