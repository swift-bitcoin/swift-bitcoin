...
swift package --allow-writing-to-directory .build generate-documentation --target BitcoinRPC --disable-indexing --transform-for-static-hosting --enable-experimental-external-link-support --hosting-base-path docc/rpc --source-service github --source-service-base-url https://github.com/swift-bitcoin/swift-bitcoin/blob/develop --checkout-path $PWD --experimental-enable-custom-templates
...
cp -rp .build/plugins/Swift-DocC/outputs/BitcoinRPC.doccarchive/. $WWW_ROOT/docc/rpc
