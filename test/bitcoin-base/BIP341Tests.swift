...
struct BIP341Tests {
    ...
    func scriptPubKey(
        ...
        if let givenScriptTree {
            let treeInfo: [(ScriptTree, Data)]
            (treeInfo, merkleRoot) = givenScriptTree.calcMerkleRoot()
            ...
            controlBlocks = treeInfo.map { leafInfo in
                ...
                return computeControlBlock(internalKey: Data(givenInternalPubkey), merkleRoot: merkleRoot, leafVersion: leafVersion, path: path)
            }
        } else {
            merkleRoot = .init()
            leafHashes = []
        ...
        let tweak = computeTapTweakHash(internalKey: Data(givenInternalPubkey), merkleRoot: merkleRoot)
        let (tweakedPubkey, _) = tweakXOnlyPublicKey(Data(givenInternalPubkey), tweak: tweak)
        ...
        */
        ...
        #expect(tweakedPubkey == Data(auxTweakedPubkey))
    ...
    func keyPathSpending() {
        ...
        for testCase in inputSpending {
            ...
            let internalPublicKey = getXOnlyPublicKey(secretKey: secretKey)
            #expect(internalPublicKey == expectedInternalPublicKey)

            let tweak = computeTapTweakHash(internalKey: internalPublicKey, merkleRoot: merkleRoot)
            #expect(tweak == expectedTweak)

            let tweakedSecretKey = tweakXOnlySecretKey(secretKey, tweak: tweak)
