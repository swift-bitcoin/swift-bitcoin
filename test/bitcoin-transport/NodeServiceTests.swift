...
struct NodeServiceTests: ~Copyable {
    ...
    init() async throws {
        ...
        let satoshi = await NodeService(blockchain: satoshiChain, config: .init(network: .regtest, feeFilterRate: 2))
    ...
    deinit {
        ...
        if satoshi != nil, let satoshiChain {
            Task {
                // self.satoshi = nil
