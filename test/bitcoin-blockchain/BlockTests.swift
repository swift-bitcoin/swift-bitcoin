...
struct BlockTests {
    ...
    func genesisBlock() async throws {
        ...
        let genesisBlockData = genesisBlock.binaryData
        #expect(genesisBlockData.hex == expectedBlockData)

        let genesisBlockRedeserialized = try TxBlock(binaryData: genesisBlockData)
    ...
    func block1() async throws {
        ...
        let expectedBlock = try TxBlock(binaryData: expectedBlockData)

        // Quick round trip check.
        #expect(expectedBlock.binaryData == expectedBlockData)
        ...
        let expectedCoinbaseTx = try BitcoinTx(binaryData: expectedCoinbaseTxData)
        ...
        #expect(coinbaseTx.binaryData == expectedCoinbaseTxData)
        #expect(block == expectedBlock)
        #expect(block.binaryData == expectedBlockData)
    ...
    func blockDateNanoseconds() throws {
        ...
        let blockRoundTrip = try TxBlock(binaryData: emptyBlock.binaryData)
