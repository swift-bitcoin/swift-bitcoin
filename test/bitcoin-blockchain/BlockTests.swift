...
struct BlockTests {
    ...
    func regtestGenesisBlock() async throws {
        ...
        let genesisBlockRedeserialized = try Block(genesisBlockData)
        #expect(genesisBlockRedeserialized == genesisBlock)
        #expect(genesisBlock.idHex == expectedBlockHash)
        ...
        #expect(genesisBlock.shortTransactionIDs(nonce: 0, dropIndices: [])[0] == expectedShortTxID)
    }
    ...
    func block1() async throws {
        ...
        let expectedBlock = try Block(expectedBlockData)
        ...
        let expectedCoinbaseTx = try Transaction(expectedCoinbaseTxData)
    ...
    func blockDateNanoseconds() throws {
        ...
        let blockRoundTrip = try Block(emptyBlock.data)
