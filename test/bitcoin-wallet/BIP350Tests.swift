...
struct BIP350Tests {
    ...
    func validAddress(address: String, script: [UInt8]) throws {
        ...
        let scriptPk = BitcoinScript([decoded!.version == 0 ? .zero : .constant(UInt8(decoded!.version)), .pushBytes(decoded!.program)]).binaryData
    ...
    func scriptToAddressCommand() throws {
        ...
        let script1 = try BitcoinScript(binaryData: script1Data)
        let script2 = try BitcoinScript(binaryData: script2Data)
